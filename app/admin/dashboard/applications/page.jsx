...
export default function ApplicationsPage() {
  ...
  const updateStatus = async (id, newStatus) => {
    ...
    try {
      ...
      if (!data) {
        throw new Error(
          'No application was updated. Check your Supabase RLS update policy.'
        )
      }
      ...
      const message =
        newStatus === 'accepted'
          ? 'Application accepted and saved'
          : newStatus === 'rejected'
            ? 'Application rejected and saved'
                    ...
                    >
                      <td className="px-6 py-4 align-top">
                        <div className="flex flex-col">
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            Age: {app.age || 'N/A'} • Grade/Year Level:{' '}
                            {app.grade_level || 'N/A'}
                          </div>
                      ...
                      <td className="px-6 py-4 align-top max-w-xs">
                        <div className="space-y-3">
                          <div>
                            ...
                            <p
                              className="text-sm text-gray-300 line-clamp-2 hover:line-clamp-none transition-all cursor-help"
                              title={app.motivation || ''}
                            >
                              "{app.motivation || 'No answer'}"
                            </p>
                          </div>
                      ...
                      <td className="px-6 py-4 text-center align-top">
                        <div className="flex flex-col gap-2 items-center w-32">
                          ...
                          {app.sample_works_urls &&
                          app.sample_works_urls.length > 0 ? (
                            <a
                      ...
                      <td className="px-6 py-4 text-right align-top">
                        <div className="flex flex-col items-end gap-2 w-32 ml-auto">
                          {status !== 'accepted' && (
                            <button
                              onClick={() => updateStatus(app.id, 'accepted')}
                              disabled={isSaving}
                              className="flex items-center gap-2 px-3 py-1.5 bg-green-500/10 text-green-400 border border-green-500/20 rounded-lg hover:bg-green-500 hover:text-white transition-all text-xs font-medium w-full justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              <CheckCircle size={14} />
                          ...
                          {status !== 'rejected' && (
                            <button
                              ...
                              disabled={isSaving}
                              className="flex items-center gap-2 px-3 py-1.5 bg-red-500/10 text-red-400 border border-red-500/20 rounded-lg hover:bg-red-500 hover:text-white transition-all text-xs font-medium w-full justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              <XCircle size={14} />
                          ...
                          {status !== 'pending' && (
                            <button
                              ...
                              disabled={isSaving}
                              className="text-xs text-gray-500 hover:text-white underline mt-1 text-center w-full disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              Reset to Pending
                            </button>
                          )}
